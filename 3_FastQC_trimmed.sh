...
IN="/scratch_isilon/groups/compgen/easensi/Captive_TFM/FastP_trim/" #path to trimmed fastqs
OUT="/scratch_isilon/groups/compgen/easensi/Captive_TFM/FastQC_trimmed"; mkdir -p $OUT #path to saving folder
p1=${IN}${fastq}_trimmed_1.fastq.gz
p2=${IN}${fastq}_trimmed_2.fastq.gz

fastqc -f fastq $p1 $p2 --outdir=$OUT #fastqc on trimmed fastqs
