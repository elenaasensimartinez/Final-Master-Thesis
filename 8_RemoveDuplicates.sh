...
OUT="/scratch_isilon/groups/compgen/easensi/Captive_TFM/RemoveDuplicates"; mkdir -p $OUT #path to saving folder
bam="/scratch_isilon/groups/compgen/easensi/Captive_TFM/AddReadGroup" #path to BAM files with added read group

java -jar $EBROOTPICARD/picard.jar MarkDuplicates -I ${bam}/${fastq}_add_read_group.bam -O ${OUT}/${fastq}_remove_duplicates.bam -MAX_FILE_HANDLES_FOR_READ_ENDS_MAP 1000 -METRICS_FILE ${OUT}/${fastq}_stats.txt -REMOVE_DUPLICATES true -ASSUME_SORTED true -MAX_RECORDS_IN_RAM 1500000 -VALIDATION_STRINGENCY SILENT -CREATE_INDEX true -COMPRESSION_LEVEL 9 #removing duplicates procedure
